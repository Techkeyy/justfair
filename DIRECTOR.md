...
- `SUBMISSION.md`: submission package (name/tagline/descriptions/problem/solution/audience/Solana/how-it-works/live-vs-fixture evidence/owner proofs/tech stack/links/video placeholder/missing-field note + 14-row internal claim table: 8 PROVEN / 2 SUPPORTED / 1 PLANNED / 3 LIMITATION).
- `SUBMISSION.md` copy clearance (2026-09-23, no product change): verdict-ownership absolute ("can never grade itself") narrowed to the proven boundary (observations-only target; engine-owned verdict; no cryptographic observation-honesty claim); market universals removed ("users discover…" → conditional production language; "no standard…" → what JustFair provides); WHY SOLANA portability absolute narrowed to Solana-specific primitives; one-liner aligned to the same class. Tech stack verified from source: `@solana/web3.js` (direct dep + directly imported in ordered DBC path — kept), `@solana/spl-token` (direct + imported), `@solana/kit` (direct + imported), `@meteora-ag/dynamic-bonding-curve-sdk` (direct + imported). Cross-copy audit: no "never grade"/monopoly/universal-behavior leftovers; Tessera=live, DBC=live, PreStocks=fixture/future-unobserved, STALE=simulation, localhost-only, no-custody upheld in copy, demo, and checklist.
