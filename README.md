...
- Data: Jupiter Swap V2 routes, Solana mainnet RPC (Token-2022 state), Nasdaq/xStocks equity references, CoinGecko SOL spot, optional Pyth streaming.
- A current fairness verdict is only produced against an eligible live reference; otherwise the check completes truthfully as `UNABLE_TO_VERIFY` (`MARKET_CLOSED_OR_AFTER_HOURS`, `STALE_REFERENCE`, or `REFERENCE_UNAVAILABLE`).
